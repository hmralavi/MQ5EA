...
strategy:
1- use two MAs
2- get into a long/short position on MAs cross
3- get out of the position as soon as the MAs get close to eachother
4- set trailing stoploss (set its offset as a variable)

copied from https://www.mql5.com/pt/code/viewcode/19546/178536/ema_6.12.mq5
   
-------------------------------------------------------------
*/

#property description "MA cross scalping"
...
input ushort   InpTrailingStep   = 2;        // Trailing Step (in pips)
input int      MA_fast_ma_period = 10;        // MA fast: averaging period
input int      MA_slow_ma_period = 30;       // MA slow: averaging period
input ulong    m_magic           = 123456; // magic number
input string InpSessionStart = "09:00";      // session start (server time)
input string InpSessionEnd = "17:00";        // session end (server time)     
//---
...
//--- create handle of the indicator iMA
   handle_iMA_fast=iMA(m_symbol.Name(),Period(),MA_fast_ma_period,0,MODE_EMA,PRICE_CLOSE);
...
//--- create handle of the indicator iMA
   handle_iMA_slow=iMA(m_symbol.Name(),Period(),MA_slow_ma_period,0,MODE_EMA,PRICE_CLOSE);
//--- if the handle is not created
...
//--- for visual
   ...
   if(count_buys==0 && count_sells==0)
     {
      datetime   _start = StringToTime(InpSessionStart);
      datetime   _finish = StringToTime(InpSessionEnd);
      datetime _currentservertime = TimeCurrent();
      if( _currentservertime>=_finish || _currentservertime<=_start)
         {
            return;
         }
      ...
      if(Slow[1]>Fast[1] && Slow[0]<Fast[0])
        {
         ...
         double sl=(ExtTrailingStop==0)?0.0:m_symbol.Bid()-ExtTrailingStop;
      ...
      //--- check condition on SELL
      else if(Slow[1]<Fast[1] && Slow[0]>Fast[0])
        {
         ...
         double sl=(ExtTrailingStop==0)?0.0:m_symbol.Ask()+ExtTrailingStop;
...
void Trailing()
  {
   ...
   for(int i=PositionsTotal()-1;i>=0;i--) // returns the number of open positions
      if(m_position.SelectByIndex(i))
         if(m_position.Symbol()==m_symbol.Name() && m_position.Magic()==m_magic)
           {
               double a = m_position.PriceCurrent();
               double b = m_position.PriceOpen();
               double c = m_position.StopLoss();
            if(m_position.PositionType()==POSITION_TYPE_BUY)
              {
               if(m_position.PriceCurrent()-m_position.StopLoss()>ExtTrailingStop+ExtTrailingStep)
                    {
